...
var LocalComputerEngine = class _LocalComputerEngine extends ComputerEngine {
  ...
  constructor(ctx, config) {
    ...
    const helperPath2 = this.resolveHelperPath();
    if (helperPath2 === void 0 || !existsSync(helperPath2)) {
      throw new Error(
        `computer-local: no computer-use daemon at ${JSON.stringify(helperPath2 ?? null)} \u2014 run 'npx @zibokapi/dsh-codex-computer-use' to build and install it (or set helperPath / DSH_COMPUTER_HELPER_PATH)`
      );
    }
    ctx.effect(() => () => {
  ...
  daemonArgv() {
    ...
    return [helperPath2, ...this.source().helperArgs ?? []];
  }
...
async function createServer(options = {}) {
  const helperPath2 = options.helperPath ?? process.env.DSH_COMPUTER_HELPER_PATH;
  if (helperPath2 === void 0 || helperPath2.length === 0) {
  ...
  await ctx.plugin(LocalComputerEngine, { helperPath: helperPath2 });
...
// src/computer-mcp/mcp.ts
var helperPath = process.argv[2];
try {
  const server = await createServer(helperPath === void 0 ? {} : { helperPath });
  await server.serve();
} catch (error) {
  const message = error instanceof Error ? error.message : String(error);
