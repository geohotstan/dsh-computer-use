...
| External MCP surface | The plugin's own MCP server | `@zibokapi/dsh-codex-computer-use/computer-mcp` stdio server |
| Approval UX | Client-side app picker with session or persistent approval, send-approval for composed messages | `@zibokapi/dsh-codex-computer-use/computer-policy` ask-per-app gate; grants persist through the settings user layer |
