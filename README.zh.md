...
`helperPath` 必须指向打包后的守护进程可执行文件；其余行可选（`@zibokapi/dsh-codex-computer-use/computer-policy` 需要挂载审批服务，例如 `@deepseek-ai/dsh-user-approval`）。可运行组合见 [`example/cordis.yml`](example/cordis.yml)。
...
- CI `install` 任务（Ubuntu）：对真实 CLI 跑文档里的安装链路——`dsh plugin --profile ci add` 分别从检出路径与打包 tarball 安装——然后组合 profile，断言 bundle 层已注册、三行已组合、且每个 `@zibokapi/dsh-codex-computer-use/*` 行都能通过已安装的包解析并加载。
